...
def print_debug(message):
    new_message = "A_estrella.py: " + message
    if debug:
        print(new_message)


class A_estrella:
    def busqueda_por_A_estrella(problema: Problema):
        nodos_expandidos = 0
        profundidad = 0
        reporte = "Generando..."
        ...
        valor = nodo_inicial.get_costo_acumulado() + nodo_inicial.calcular_heuristica()
        cola_prioridad.put((valor, nodo_inicial))

        # while not len(cola_prioridad) == 0:
        while not cola_prioridad.empty():
            print_debug("La cola de prioridad actual es {}".format(
                str(cola_prioridad)))
            valor, nodo = cola_prioridad.get()  # Saca un nodo de la cola
            print_debug("{}".format(str(valor)))
            print_debug("{}".format(str(nodo)))
            print_debug("{}".format(str(valor), str(nodo)))

            if nodo.es_meta():
                camino = A_estrella.reconstruir_camino(nodo)
                reporte = A_estrella.generar_reporte(
...
class Test():
    ...
    def start():
        ...
        print(A_estrella.busqueda_por_A_estrella(problema))
